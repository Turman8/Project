...
typedef struct {
    ...
    XEcg_classify_trained_uio_map maps[ MAX_UIO_MAPS ];
} XEcg_classify_trained_uio_info;

/***************** Variable Definitions **************************************/
static XEcg_classify_trained_uio_info uio_info;
...
static int uio_info_read_name(XEcg_classify_trained_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XEcg_classify_trained_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XEcg_classify_trained_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XEcg_classify_trained_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XEcg_classify_trained_Initialize(XEcg_classify_trained *InstancePtr, const char* InstanceName) {
	XEcg_classify_trained_uio_info *InfoPtr = &uio_info;
